...
const SubmitPage: React.FC = () => {
  ...
  const [courseProblem, setCourseProblem] = useState<CourseProblem | null>(
    null,
  );
    ...
    const fetchCourseProblem = async () => {
      try {
        const response = await axiosInstance.get(
          `problem/course/${courseProblemId}`,
        );
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10">
      ...
      <p className="text-gray-700 dark:text-gray-300 mb-6">
        {courseProblem.problem.description}
      </p>
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
        Course: {courseProblem.course.name} | Due:{" "}
        {new Date(courseProblem.dueDate).toLocaleString()}
      </p>
